...
String listenerDeviceName = "7tally-1";
...
char tallyarbiter_host[40] = "192.168.1.12"; //IP address of the Tally Arbiter Server
char tallyarbiter_port[6] = "4455";
char tallyarbiter_name[20] = "";
...
void otaErrorHandler(ota_error_t error) {
...
void setup() {
  ...
  listenerDeviceName = "7tally-" + String((uint16_t)(chipid>>32)) + String((uint32_t)chipid);

  if(preferences.getString("taName").length() > 0){
    listenerDeviceName = preferences.getString("taName");
  }
  ...
  if(preferences.getString("taName").length() > 0){
    String newName = preferences.getString("taName");
    logger("Setting TallyArbiter client name as" + newName, "info-quiet");
    newName.toCharArray(tallyarbiter_port, 20);
  }
...
void connectToNetwork() {
  ...
  WiFiManagerParameter custom_taName("taName", "Name", tallyarbiter_name, 6);

  wm.addParameter(&custom_taServer);
  wm.addParameter(&custom_taPort);
  wm.addParameter(&custom_taName);
...
void connectToServer() {
  ...
  socket.begin(String(tallyarbiter_host), atoi(tallyarbiter_port));
}
